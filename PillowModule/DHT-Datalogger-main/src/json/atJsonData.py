...
class atJsonData():
    ...
    """
    file_json = None
    DATA_PATH = "Data.json"#"json/Data.json"
    BACK_UP_DATA_PATH = "BackUpData.json"#"json/BackUpData.json"
    DEFAULT_DATA_PATH = "DefaultData.json"#"json/DefaultData.json"
    SCREEN_NAME_PATH = "ScreenName.json"#"json/ScreenName.json"
    _is_saving = False
    ...
    def clone_Sensor(self, ID):
        ...
        self.data["Sensors"]["Number"] = new_Sensor_Index
        new_Sensor_Index_str = str(new_Sensor_Index)
        self.data["Sensors"]["List"][new_Sensor_Index_str] = self.data["Sensors"]["List"][str(
            ID)]
        self.save("Sensors")
    ...
    def clone_Relay(self, ID):
        ...
        self.data["Relays"]["Number"] = new_Relay_Index
        new_Relay_Index_str = str(new_Relay_Index)
        self.data["Relays"]["List"][new_Relay_Index_str] = self.data["Relays"]["List"][str(
            ID)]
        self.save("Relays")
    ...
    def delete_Sensor(self, ID):
        ...
        for count in range(1, old_list_length):
            if count >= int(ID):
                self.data["Sensors"]["List"][str(
                    count)] = self.data["Sensors"]["List"][str(count+1)]
    ...
    def delete_Relay(self, ID):
        ...
        for count in range(1, old_list_length):
            if count >= int(ID):
                self.data["Relays"]["List"][str(
                    count)] = self.data["Relays"]["List"][str(count+1)]
...
def main():
    atData = atJsonData()
    # for key in atData.data:
    #     print(key)
    # print(atData.data["Notification"])
