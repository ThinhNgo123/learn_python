...
class FrameMenuSensorEdit:
    ...
    def event_handling(self, event):
        if event == FrameMenuSensorEdit.ACTION_HELP:
            frame = FrameHelp(self.hmi_screen)
            self.hmi_screen.push_frame(frame)
        elif event == FrameMenuSensorEdit.ACTION_UP:
            ...
            if self.edit_status:
                self.edit.event_handling(FrameMenuSensorEdit.ACTION_BACK)
            else:
                self.hmi_screen.pop_frame()
        elif event == FrameMenuSensorEdit.ACTION_NEXT:
            if self.edit_status:
                self.edit.event_handling(FrameMenuSensorEdit.ACTION_NEXT)
        elif event == FrameMenuSensorEdit.ACTION_OK:
            if len(self.edit.pages):
                element = self.edit.get_element()
        ...
        elif event == FrameMenuSensorEdit.ACTION_DELETE:
            self.edit.event_handling("delete")
        elif event == FrameMenuSensorEdit.ACTION_WRITE:
            self.edit.event_handling("add")
