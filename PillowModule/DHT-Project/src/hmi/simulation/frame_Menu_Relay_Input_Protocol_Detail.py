...
class FrameMenuRelayInputProtocolDetail:
    ACTION_UP = "up"
    ACTION_DOWN = "down"
    ACTION_BACK = "left"
    ACTION_OK = "enter"
    ...
    def __init__(self, hmi_screen):
        ...
        self.field.set_title(center_x=FrameMenuRelayInputProtocolDetail.TITLE_CENTER_X, center_y=FrameMenuRelayInputProtocolDetail.TITLE_CENTER_Y)
        self.field.set_first_element_position(FrameMenuRelayInputProtocolDetail.FIRST_FIELD_POS_X, FrameMenuRelayInputProtocolDetail.FIRST_FIELD_POS_Y)
        self.edit = MenuEdit(self)
        self.edit.set_first_element_position(FrameMenuRelayInputProtocolDetail.FIRST_EDIT_POS_X, FrameMenuRelayInputProtocolDetail.FIRST_EDIT_POS_Y)
        ...
        self.edit.add_element("edit", "asdnmasd.com")
        self.edit.add_element("edit", "1883")
        self.edit.add_element("edit", "Relay trigger")
        self.edit.add_element("edit", "user")
        self.edit.add_element("edit", "user")
    ...
    def event_handling(self, event):
        if event == FrameMenuRelayInputProtocolDetail.ACTION_HELP:
            frame = FrameHelp(self.hmi_screen)
            self.hmi_screen.push_frame(frame)
        elif event == FrameMenuRelayInputProtocolDetail.ACTION_UP:
            if not self.edit_status:
                self.field.event_handling("up")
            self.edit.event_handling("up")
        elif event == FrameMenuRelayInputProtocolDetail.ACTION_DOWN:
            if not self.edit_status:
                self.field.event_handling("down")
            self.edit.event_handling("down")
        elif event == FrameMenuRelayInputProtocolDetail.ACTION_BACK:
            if self.edit_status:
                self.edit.set_edit_status(False)
                self.edit_status = False
            else:
                self.hmi_screen.pop_frame()
        elif event == FrameMenuRelayInputProtocolDetail.ACTION_OK:
            ...
            if event == FrameMenuRelayInputProtocolDetail.ACTION_DELETE:
...
def main():
    test = FrameMenuRelayInputProtocolDetail("hmi")
    test.get_image().show()
