...
class atFrame_Device_Type(atFrame_Edit):
    def __init__(self,) -> None:


        name= atData.screen_names["Setting"]["Device"]["Type"]["screen name"]
        super().__init__(
            name = name,
            detail= atData.data["Device"]["Type"]
        )
    ...
    def save(self):
        atData.data["Device"]["Type"] = self.detail
        atData.save("Device")
    ...
    def load(self):
        if self.name == Current_Screen.name:
            if self.rended == False:
                self.rended = True
                self.set_detail(atData.data["Device"]["Type"])
...
atScreen_Device_Type = atFrame_Device_Type()

if __name__ == "__main__":
    Current_Screen.set_name(atScreen_Device_Type.name)
    while 1:
