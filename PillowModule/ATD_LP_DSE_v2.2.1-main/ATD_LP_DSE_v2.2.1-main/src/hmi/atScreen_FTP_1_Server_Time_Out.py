...
class atFrame_FTP_1_Server_Time_Out(atFrame_Edit):
    def __init__(self,) -> None:



        name= atData.screen_names["Setting"]["FTP_Servers"]["FTP1"]["Time Out"]["screen name"]
        super().__init__(
            name = name,
            detail= str(atData.data["FTP_Servers"]["FTP1"]["Time Out"]),
            edit_mode="Number"
        )
    ...
    def save_callback(self):
        try:
            atData.data["FTP_Servers"]["FTP1"]["Time Out"] = int(self.detail)
            atData.save()
            notification = "FTP:Time Out:" + self.detail
        except:
    ...
    def load(self):
        if self.name == Current_Screen.name:
            if self.rended == False:
                self.rended = True
                self.detail = str(atData.data["FTP_Servers"]["FTP1"]["Time Out"])
...
atScreen_FTP_1_Server_Time_Out = atFrame_FTP_1_Server_Time_Out()

if __name__ == "__main__":
    Current_Screen.set_name(atScreen_FTP_1_Server_Time_Out.name)
    while 1:
