...
class atFrame_Relay_Detail_Config_Name(atFrame_Edit):
    def __init__(self,) -> None:


        name= atData.screen_names["Setting"]["Relays"]["Relay_Detail"]["Config"]["Name"]["screen name"]

        self.Setting_Relay_ID = atData.data["Relays"]["Setting Relay"]
        super().__init__(
            name = name,
            # detail= atData.data["Relays"]["List"][self.Setting_Relay_ID]["Name"],
            edit_mode= "String"
        )
    ...
    def save_callback(self):
        self.Setting_Relay_ID = atData.data["Relays"]["Setting Relay"]
        atData.data["Relays"]["List"][self.Setting_Relay_ID]["Name"] = self.detail
        atData.save("Relays")
        notification = "R" + self.Setting_Relay_ID + ":Name:"+self.detail
...
atScreen_Relay_Detail_Config_Name = atFrame_Relay_Detail_Config_Name()

if __name__ == "__main__":
    Current_Screen.set_name(atScreen_Relay_Detail_Config_Name.name)
    while 1:
