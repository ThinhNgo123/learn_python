...
class atFrame_Sensor_Detail_Config_Limit_Upper(atFrame_Edit):
    def __init__(self,) -> None:


        name= atData.screen_names["Setting"]["Sensors"]["Sensor_Detail"]["Config"]["Limit"]["Upper"]["screen name"]

        self.Setting_Sensor_ID = atData.data["Sensors"]["Setting Sensor"]
        super().__init__(
            name = name,
            # detail= str(atData.data["Sensors"]["List"][self.Setting_Sensor_ID]["Limit"]["Upper"]),
            edit_mode="Number"
        )
    ...
    def save_callback(self):
        try:
            ...
            self.Setting_Sensor_ID = atData.data["Sensors"]["Setting Sensor"]
            atData.data["Sensors"]["List"][self.Setting_Sensor_ID]["Limit"]["Upper"] = float(self.detail)
            atData.save("Sensors")
            notification = "S" + self.Setting_Sensor_ID +":LU:"+ self.detail
...
atScreen_Sensor_Detail_Config_Limit_Upper = atFrame_Sensor_Detail_Config_Limit_Upper()

if __name__ == "__main__":
    Current_Screen.set_name(atScreen_Sensor_Detail_Config_Limit_Upper.name)
    while 1:
