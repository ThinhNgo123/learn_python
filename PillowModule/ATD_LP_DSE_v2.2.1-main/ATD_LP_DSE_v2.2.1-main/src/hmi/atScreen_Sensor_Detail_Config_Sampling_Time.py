...
class atFrame_Sensor_Detail_Config_Sampling_Time(atFrame_Edit):
    def __init__(self,) -> None:



        name= atData.screen_names["Setting"]["Sensors"]["Sensor_Detail"]["Config"]["Sampling_Time"]["screen name"]

        self.Setting_Sensor_ID = atData.data["Sensors"]["Setting Sensor"]
        super().__init__(
            name = name,
            # detail= str(atData.data["Sensors"]["List"][self.Setting_Sensor_ID]["Sampling Time"]),
            edit_mode= "Number"
        )
    ...
    def save_callback(self):
        try:
            self.Setting_Sensor_ID = atData.data["Sensors"]["Setting Sensor"]
            atData.data["Sensors"]["List"][self.Setting_Sensor_ID]["Sampling Time"] = int(self.detail)
            atData.save("Sensors")
            notification = "S" + self.Setting_Sensor_ID + ":ST:" + self.detail
        except:
...
atScreen_Sensor_Detail_Config_Sampling_Time = atFrame_Sensor_Detail_Config_Sampling_Time()

if __name__ == "__main__":
    Current_Screen.set_name(atScreen_Sensor_Detail_Config_Sampling_Time.name)
    while 1:
