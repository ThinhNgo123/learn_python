...
class atFrame_Sensor_Detail_Config_Source_MBTCP_Config_IP(atFrame_Edit):
    def __init__(self,) -> None:

        super().__init__(
            name = atData.screen_names["Setting"]["Sensors"]["Sensor_Detail"]["Config"]["Source"]["Modbus_TCP"]["Config"]["IP"]["screen name"],
            # detail= atData.data["Sensors"]["List"][self.Setting_Sensor_ID]["Source"]["Modbus TCP/IP config"]["IP"],
            edit_mode= "Number"
        )
    ...
    def save_callback(self):
        try:
            ...
            atData.data["Sensors"]["List"][self.Setting_Sensor_ID]["Source"]["Modbus TCP/IP config"]["IP"] = self.detail
            atData.save("Sensors")
            notification = "MBTCP:IP:" + self.detail
        except:
...
atScreen_Sensor_Detail_Config_Source_MBTCP_Config_IP = atFrame_Sensor_Detail_Config_Source_MBTCP_Config_IP()

if __name__ == "__main__":
    Current_Screen.set_name(atScreen_Sensor_Detail_Config_Source_MBTCP_Config_IP.name)
    while 1:
