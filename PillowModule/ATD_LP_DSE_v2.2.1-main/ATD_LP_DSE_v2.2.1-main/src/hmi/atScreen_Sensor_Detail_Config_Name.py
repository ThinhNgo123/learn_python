...
class atFrame_Sensor_Detail_Config_Name(atFrame_Edit):
    def __init__(self,) -> None:


        name= atData.screen_names["Setting"]["Sensors"]["Sensor_Detail"]["Config"]["Name"]["screen name"]

        self.Setting_Sensor_ID = atData.data["Sensors"]["Setting Sensor"]
        super().__init__(
            name = name,
            # detail= atData.data["Sensors"]["List"][self.Setting_Sensor_ID]["Name"],
            edit_mode= "String"
        )
    ...
    def save_callback(self):
        self.Setting_Sensor_ID = atData.data["Sensors"]["Setting Sensor"]
        atData.data["Sensors"]["List"][self.Setting_Sensor_ID]["Name"] = self.detail
        atData.save("Sensors")
        notification = "S" + self.Setting_Sensor_ID + ":Name:"+self.detail
...
atScreen_Sensor_Detail_Config_Name = atFrame_Sensor_Detail_Config_Name()

if __name__ == "__main__":
    Current_Screen.set_name(atScreen_Sensor_Detail_Config_Name.name)
    while 1:
