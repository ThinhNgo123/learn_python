...
class atFrame_Device_Time_Zone(atFrame_Edit):
    def __init__(self,) -> None:


        name= atData.screen_names["Setting"]["Device"]["Time_Zone"]["screen name"]
        super().__init__(
            name = name,
            detail= atData.data["Device"]["Time_Zone"]
        )
    ...
    def save(self):
        if " " in self.detail:
            ...
            atData.data["Device"]["Time_Zone"] = self.detail
            atData.save("Device")
    ...
    def load(self):
        if self.name == Current_Screen.name:
            if self.rended == False:
                self.rended = True
                self.set_detail(atData.data["Device"]["Time_Zone"])
...
atScreen_Device_Time_Zone = atFrame_Device_Time_Zone()

if __name__ == "__main__":
    Current_Screen.set_name(atScreen_Device_Time_Zone.name)
    while 1:
