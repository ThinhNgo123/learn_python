...
from atRestoreUSBDatabase import atRestore_USB_Database
...
class atFrame_Advances_Restore_USB_Database(atFrame_Menu):
    def __init__(self,) -> None:



        name= atData.screen_names["Setting"]["Advances"]["Restore_USB"]["Database"]["screen name"]
    ...
    def screen_function(self):
        ...
        if (self.get_chosen_submenu() == "Yes"):
            successful_restore = atRestore_USB_Database.restore()
            if(successful_restore):
                self.set_notification("Restored")
                sleep(0.5)
                self.set_notification("---")
            else:
                self.set_notification("There is no USB")
                sleep(0.5)
                self.set_notification("---")
        else:
...
atScreen_Advances_Restore_USB_Database = atFrame_Advances_Restore_USB_Database()

if __name__ == "__main__":
    Current_Screen.set_name(atScreen_Advances_Restore_USB_Database.name)
    while 1:
