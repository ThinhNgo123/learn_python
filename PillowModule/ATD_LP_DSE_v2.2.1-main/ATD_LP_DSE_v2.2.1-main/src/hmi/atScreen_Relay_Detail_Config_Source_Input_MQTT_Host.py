...
class atFrame_Relay_Detail_Config_Source_Input_MQTT_Host(atFrame_Edit):
    def __init__(self,) -> None:


        name= atData.screen_names["Setting"]["Relays"]["Relay_Detail"]["Config"]["Source"]["Input"]["MQTT"]["Host"]["screen name"]
    ...
    def save(self):
        if " " in self.detail:
            ...
            atData.data["Relays"]["List"][self.Setting_Relay_ID]["Source"]["Input"]["MQTT"]["Host"] = self.detail
            atData.save("Relays")
        sleep(0.5)
    ...
    def load(self):
        if self.name == Current_Screen.name:
            if self.rended == False:
                ...
                self.Setting_Relay_ID = atData.data["Relays"]["Setting Relay"]
                self.set_detail(
                    detail=atData.data["Relays"]["List"][self.Setting_Relay_ID]["Source"]["Input"]["MQTT"]["Host"],
                )
...
atScreen_Relay_Detail_Config_Source_Input_MQTT_Host = atFrame_Relay_Detail_Config_Source_Input_MQTT_Host()

if __name__ == "__main__":
    Current_Screen.set_name(atScreen_Relay_Detail_Config_Source_Input_MQTT_Host.name)
    while 1:
