...
from atBackUpUSBSetting import atBuck_Up_USB_Setting
...
class atFrame_Advances_Backup_USB_Setting(atFrame_Menu):
    def __init__(self,) -> None:


        name= atData.screen_names["Setting"]["Advances"]["Backup_USB"]["Setting"]["screen name"]
    ...
    def screen_function(self):
        self.set_notification("Saving")
        if (self.get_chosen_submenu() == "Yes"):
            successful_backup = atBuck_Up_USB_Setting.save()
...
atScreen_Advances_Backup_USB_Setting = atFrame_Advances_Backup_USB_Setting()

if __name__ == "__main__":
    Current_Screen.set_name(atScreen_Advances_Backup_USB_Setting.name)
    while 1:
