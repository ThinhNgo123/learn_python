...
sys.path.insert(0,'func/backup')
from atBackUpUSBDatabase import atBuck_Up_USB_Database
...
class atFrame_Advances_Backup_USB_Database(atFrame_Menu):
    def __init__(self,) -> None:


        name= atData.screen_names["Setting"]["Advances"]["Backup_USB"]["Database"]["screen name"]
    ...
    def screen_function(self):
        self.set_notification("Backing up")
        if (self.get_chosen_submenu() == "Yes"):
            successful_backup = atBuck_Up_USB_Database.save()
            if(successful_backup):
                self.set_notification("Backed up")
                sleep(0.5)
                self.set_notification("---")
            else:
                self.set_notification("These is no USB")
                sleep(0.5)
                self.set_notification("---")
        else:
...
atScreen_Advances_Backup_USB_Database = atFrame_Advances_Backup_USB_Database()

if __name__ == "__main__":
    Current_Screen.set_name(atScreen_Advances_Backup_USB_Database.name)
    while 1:
