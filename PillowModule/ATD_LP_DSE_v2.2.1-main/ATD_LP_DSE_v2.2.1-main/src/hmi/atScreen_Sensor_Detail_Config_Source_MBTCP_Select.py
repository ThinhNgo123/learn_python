...
class atFrame_Sensor_Detail_Config_Source_MBTCP_Select(atFrame_Menu):
    def __init__(self,) -> None:



        super().__init__(
            name = atData.screen_names["Setting"]["Sensors"]["Sensor_Detail"]["Config"]["Source"]["Modbus_TCP"]["Select"]["screen name"],
            list=[
                "Yes",
                "No"
            ]
    ...
    def new_OK_callback(self):
        if self.get_chosen_submenu() == "Yes":
            ...
            atData.data["Sensors"]["List"][self.Setting_Sensor_ID]["Source"]["Protocol"] = "Modbus TCP"
            atData.save("Sensors")
...
atScreen_Sensor_Detail_Config_Source_MBTCP_Select = atFrame_Sensor_Detail_Config_Source_MBTCP_Select()

if __name__ == "__main__":
    Current_Screen.set_name(atScreen_Sensor_Detail_Config_Source_MBTCP_Select.name)
    while 1:
