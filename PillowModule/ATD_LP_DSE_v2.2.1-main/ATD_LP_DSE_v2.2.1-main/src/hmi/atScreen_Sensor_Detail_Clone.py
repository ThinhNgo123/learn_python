...
class atFrame_Sensor_Detail_Clone(atFrame_Menu):
    def __init__(self,) -> None:


        name= atData.screen_names["Setting"]["Sensors"]["Sensor_Detail"]["Clone"]["screen name"]
        super().__init__(
            name = name,
            list= [
                "No",
    ...
    def screen_function(self):
        atData.clone_Sensor(atData.data["Sensors"]["Setting Sensor"])
...
atScreen_Sensor_Detail_Clone = atFrame_Sensor_Detail_Clone()

if __name__ == "__main__":
    Current_Screen.set_name(atScreen_Sensor_Detail_Clone.name)
    while 1:
