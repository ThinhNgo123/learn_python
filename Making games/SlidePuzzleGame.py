...
def drawTile(tilex, tiley, number, adjx=0, adjy=0):
	...
	# textSurf, textRect = makeText(str(number), TEXTCOLOR, TILECOLOR, top, left)
	# textRect.top = top + ((TILESIZE - textRect.width -2) / 2) + adjy
	# textRect.left = left + ((TILESIZE - textRect.height -2) / 2) + adjx
...
def generateNewPuzzle(numSlides):
	sequence = []
	board = getStartingBoard() #[[1,5,9,13], ...[4,8,12,None]]
	drawBoard(board, '')
	pygame.display.update()
